...
- **Auth**: Clerk or Lucia with magic link login
...
## Phase 1: Setup & Infrastructure (Week 1) ⏸ PENDING

| ID  | Task                                 | Status    | Priority | Details                                                          |
| --- | ------------------------------------ | --------- | -------- | ---------------------------------------------------------------- |
| 1.1 | Initialize Astro project             | ⏸ PENDING | HIGH     | `npm create astro@latest`, TypeScript strict, TailwindCSS config |
| 1.2 | Set up Convex backend                | ⏸ PENDING | HIGH     | `npx convex dev`, project init, connect to Astro                 |
| 1.3 | Configure Vercel deployment          | ⏸ PENDING | HIGH     | Astro adapter for Vercel, environment variables, preview deploys |
| 1.4 | Integrate auth (Clerk magic link)    | ⏸ PENDING | HIGH     | Clerk setup, magic link flow, user session in Convex             |
| 1.5 | Install and wire core libraries      | ⏸ PENDING | MEDIUM   | geolib, zod, nanostores, date-fns                                |
| 1.6 | Basic project documentation & README | ⏸ PENDING | LOW      | Dev setup instructions, architecture overview, env vars          |

## Phase 2: Core Data Models (Week 1–2) ⏸ PENDING

| ID  | Task                                     | Status    | Priority | Details                                                                                          |
| --- | ---------------------------------------- | --------- | -------- | ------------------------------------------------------------------------------------------------ |
| 2.1 | Define Convex schema: users table        | ⏸ PENDING | HIGH     | email, name, lat, lng, trustScore, createdAt; index by_email                                     |
| 2.2 | Define Convex schema: requests table     | ⏸ PENDING | HIGH     | title, desc, requesterId, helperId, status, lat/lng, timestamps; indexes by_status, by_requester |
| 2.3 | Define Convex schema: interactions table | ⏸ PENDING | HIGH     | requesterId, helperId, requestId, outcome, createdAt; indexes by_requester, by_helper, by_pair   |
| 2.4 | Define Convex schema: messages table     | ⏸ PENDING | MEDIUM   | requestId, senderId, body, createdAt; index by_request                                           |
| 2.5 | User creation mutation on first auth     | ⏸ PENDING | HIGH     | Upsert user record after magic link sign-in; store exact lat/lng                                 |

## Phase 3: Core Request Features (Week 2) ⏸ PENDING

| ID  | Task                                | Status    | Priority | Details                                                                         |
| --- | ----------------------------------- | --------- | -------- | ------------------------------------------------------------------------------- |
| 3.1 | Create request mutation             | ⏸ PENDING | HIGH     | Insert request with status=open; validate with Zod                              |
| 3.2 | Geo-filtered open feed query        | ⏸ PENDING | HIGH     | Bounding box ~1km, filter status=open, exclude own, sort by distance via geolib |
| 3.3 | Accept request mutation (race-safe) | ⏸ PENDING | HIGH     | Verify status=open, assign helperId, transition to accepted                     |
| 3.4 | Complete request mutation           | ⏸ PENDING | HIGH     | Validate participants, set status=completed, set completedAt                    |
| 3.5 | Cancel request mutation             | ⏸ PENDING | HIGH     | Only requester can cancel; transition open → cancelled                          |

## Phase 4: Trust System (Week 2–3) ⏸ PENDING

| ID  | Task                              | Status    | Priority | Details                                                                |
| --- | --------------------------------- | --------- | -------- | ---------------------------------------------------------------------- |
| 4.1 | Interaction logging on completion | ⏸ PENDING | HIGH     | Insert interaction record (outcome=completed) inside complete mutation |
| 4.2 | Increment helper trustScore       | ⏸ PENDING | HIGH     | Atomic increment on users.trustScore when interaction is logged        |
| 4.3 | Expose trustScore on user profile | ⏸ PENDING | MEDIUM   | Query to fetch public user profile with trust score                    |
...
| 7.6 | Auth flow pages (sign-in, magic link)  | ⏸ PENDING | HIGH     | Clerk-powered sign-in page; redirect after auth             |
...
| 8.4 | Auth edge cases                        | ⏸ PENDING | HIGH     | Expired magic link, unauthenticated access to protected routes    |
